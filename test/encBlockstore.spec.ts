...
import { FlatDirectory } from 'blockstore-fs/sharding'
...
describe('EncBlockstore', () => {
  ...
  const blockMasterSalt = globalThis.crypto.getRandomValues(new Uint8Array(16)) // 128-bit salt
  const cidMasterSalt = globalThis.crypto.getRandomValues(new Uint8Array(16)) // 128-bit salt
  ...
  beforeEach(async () => {
    ...
    encStore = new EncBlockstore(password, blockMasterSalt, cidMasterSalt, mockStore, init)
    await encStore.open()
  })

  it('should encrypt and decrypt a CID correctly', async () => {
    const cid = await generateRandomCID()

    // eslint-disable-next-line @typescript-eslint/dot-notation
    const encCID = await encStore['encryptCID'](cid)
    // eslint-disable-next-line @typescript-eslint/dot-notation
    const decCID = await encStore['decryptCID'](encCID)

    expect(decCID).toEqual(cid)
  })

  it('encrypted CID should be a valid ', async () => {
    const cid = await generateRandomCID()
    const specs = CID.inspectBytes(cid.bytes)
    console.log('orig specs', specs)

    // eslint-disable-next-line @typescript-eslint/dot-notation
    const encCID = await encStore['encryptCID'](cid)

    expect(() => {
      const specs = CID.inspectBytes(encCID.bytes)
      console.log('specs', specs)
      const prefixSize = specs.size - specs.multihashSize
      console.log('prefixSize', prefixSize)
      const multihashBytes = coerce(
        encCID.bytes.subarray(prefixSize, prefixSize + specs.multihashSize)
      )
      console.log('multihashBytes', multihashBytes)
      if (multihashBytes.byteLength !== specs.multihashSize) {
        throw new Error('Incorrect length')
      }

      // CID.decodeFirst(encCID.multihash.bytes)
    }).not.toThrow()
  })
  ...
  it('should handle encryption and decryption correctly', async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/dot-notation
    const storageCID = await encStore['encryptCID'](cid)
  ...
  it('should handle incorrect password decryption attempts gracefully', async () => {
    ...
    const wrongPasswordStore = new EncBlockstore(wrongPassword, blockMasterSalt, cidMasterSalt, mockStore)
    await wrongPasswordStore.open()
  ...
  it('should handle incorrect salt decryption attempts gracefully', async () => {
    ...
    const wrongPasswordStore = new EncBlockstore(password, wrongMasterSalt, cidMasterSalt, mockStore)
    await wrongPasswordStore.open()
  ...
  it('should allow opening and closing multiple times and data should persist', async () => {
    const data = new TextEncoder().encode('Block to persist')
    const cid = await generateRandomCID()
    await encStore.put(cid, data)
    await encStore.close()

    const encStore2 = new EncBlockstore(password, blockMasterSalt, cidMasterSalt, mockStore)
    await encStore2.open()

    const retrievedData = await encStore2.get(cid)
    expect(retrievedData).toEqual(data)
  })

  describe('Memory interface-blockstore', () => {
    interfaceBlockstoreTests({
      setup: async () => {
        const store = new MemoryBlockstore()
        const encBlockstore = new EncBlockstore(password, blockMasterSalt, cidMasterSalt, store)
        await encBlockstore.open()

        return encBlockstore
      },
  ...
  describe('Level interface-blockstore', () => {
    const tmpPath = path.join(os.tmpdir(), `test-${Math.random()}`)

    interfaceBlockstoreTests({
      setup: async () => {
        const store = new LevelBlockstore(tmpPath)
        const encBlockstore = new EncBlockstore(password, blockMasterSalt, cidMasterSalt, store)
        await encBlockstore.open()
  ...
  describe.only('FS interface-blockstore', () => {
    const tmpPath = path.join(os.tmpdir(), `test-${Math.random()}`)

    interfaceBlockstoreTests({
      setup: async () => {
        // const store = new FsBlockstore(tmpPath)
        const store = new FsBlockstore(tmpPath, {
